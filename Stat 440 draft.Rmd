...
#Model 1 mixed model and ICC results
set.seed(123455)
...
fixedmod <- function(y,x,S,burnin, a, b, mu0, tau2){
  ...
  for(s in 1:S){
    ...
    #variance
    v <- XX*sig2inv 
    diag(v) <- diag(v) + 1/tau2
    v <- chol2inv(chol(v))
    
    #mean
    m <- v %*% (sig2inv*Xy + mu0tau2)
    
    # simulate from FC
    beta <- drop(m +  t(chol(v)) %*% rnorm(p))
    
    ### update sig2inv from full conditional
    sig2inv <- rgamma(1, a + n/2,  b + 0.5*sum((y-X%*%beta)^2))
...
#fixed model results
 ...
 x = burrito %>%
     select(-"overall" )
# make x a numeric matrix
x$Reviewer = match(x$Reviewer, unique(x$Reviewer))
...
fixedmod(y,x,S,burnin, a, b, mu0, tau2)
